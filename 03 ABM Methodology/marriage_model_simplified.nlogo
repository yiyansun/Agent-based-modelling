; Coded by Thomas Oléron Evans
; A much simplified version or the marriage model from the ODD description provided by Railsback & Grimm (2011)
...
[
  ...
  num-babies                 ; Number of children born each time step
...
to childbirth ; observer context
  ...
  ask n-of num-babies (patches with [pxcor = 0])
  [sprout 1 [initialise-baby]]
...
to marriage ; observer context
  ...
  ask one-of (turtles with [(age >= min-marriage-age) and (married? = false)])
    [
      set partner (one-of (turtles with [(married? = false) and (gender != ([gender] of myself)) and (age >= min-marriage-age)]))
        if partner != nobody
        [
          set married? true
...
to setup-plotting ; observer context
