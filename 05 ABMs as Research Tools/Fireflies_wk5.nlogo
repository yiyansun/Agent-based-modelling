...
to setup
  ...
  set is-sync? False  ; NEW LINES
  set sync-time -1     ; NEW LINES
...
to go
  ...
  if (is-sync? = False) [check-sync]  ; NEW LINES
...
to check-sync  ; NEW LINES

  let flash-number (count turtles with [color = yellow])
  if number = flash-number
  [
    set is-sync? True
    set sync-time ticks
  ]
...
@#$#@#$#@
<experiments>
  <experiment name="Week 5" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <exitCondition>is-sync? = True</exitCondition>
    <metric>sync-time</metric>
    ...
    <steppedValueSet variable="cycle-length" first="10" step="1" last="14"/>
    <steppedValueSet variable="flash-length" first="1" step="1" last="5"/>
