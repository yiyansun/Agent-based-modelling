...
to move
  ...
  [
    ask turtles
    [
      set next-patch one-of ((neighbors4 with [pcolor != white and any? turtles-here = false]) with-min [dist-allexits])
      carefully
      [
        face next-patch
        move-to next-patch
...
to show-evac-routes [exit-num]
  ...
  if exit-num = 1
  [
    ask patches with [dist-exit1 > 0]
    [
      set pcolor scale-color yellow dist-exit1 (max [dist-exit1] of patches) 0
    ]
  ]

  if exit-num = 2
  [
    ask patches with [dist-exit2 > 0]
    [
      set pcolor scale-color yellow dist-exit2 (max [dist-exit2] of patches) 0
    ]
  ]

    if exit-num = 3
  [
    ask patches with [dist-exit3 > 0]
    [
      set pcolor scale-color yellow dist-exit3 (max [dist-exit3] of patches) 0
    ]
  ]

    if exit-num = 0
  [
    ask patches with [dist-allexits > 0]
    [
      set pcolor scale-color yellow dist-allexits (max [dist-allexits] of patches) 0
    ]
  ]
...
"" ""
PENS
"default" 1.0 0 -13840069 true "" "plot count-evacuees1"
"pen-1" 1.0 0 -13345367 true "" "plot count-evacuees2"
"pen-2" 1.0 0 -2674135 true "" "plot count-evacuees3"
