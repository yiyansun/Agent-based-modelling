...
to alarm-sounds
  ...
  [
    set alarm? true
    ask patches with [pxcor > 14 and pxcor < 22 and pycor > -22 and pycor < -19]
    [set pcolor orange]

    ask turtles
    [
      set chosen-exit 0
      let current-dist (item 0 dist-exit-list)
      foreach [1 2]
      [i ->
        if (item i dist-exit-list < current-dist) and (item i exit-knowledge-list = true)
        [set chosen-exit i]
      ]
    ]
...
to setup-patches
  ...
  [
    set staffonly? false
    set dist-exit-list [ 1000001 1000001 1000001 ]
    set selected-list ["empty" "empty" "empty"]
  ]
...
to create-agents
  ...
  [
    ...
    let know-exit-2 ((random 2) = 1)
    let know-exit-3 ((random 2) = 1)
    set exit-knowledge-list (list (true) (know-exit-2) (know-exit-3))
  ]
  ...
  [
    ...
    set exit-knowledge-list [true true true]
  ]
...
to define-evacuation-routes
  ...
  (foreach [green blue red] [0 1 2]
    [[colour i] ->

  ask patches with [pcolor = colour]
  [
    set dist-exit-list (replace-item i dist-exit-list 0)
    set selected-list (replace-item i selected-list false)
  ]
    ])

  ask patches with [pcolor = black]
  [
    set dist-exit-list [ 1000000 1000000 1000000 ]
    set selected-list [ false false false ]
  ]

  foreach [0 1 2]
  [i ->
  while [any? (patches with [(item i dist-exit-list) = 1000000])]
  [
    let possible-patches (patches with [(item i selected-list) = false])
    let chosen-patch (one-of possible-patches with-min [(item i dist-exit-list)])

    ask chosen-patch
    [
      set selected-list (replace-item i selected-list true)
      ask (neighbors4 with [(item i dist-exit-list) = 1000000])
      [set dist-exit-list (replace-item i dist-exit-list (1 + [(item i dist-exit-list)] of myself))]
    ]
  ]
  ]

  ask patches
  [set dist-allexits min dist-exit-list]
...
to move
  ...
  ifelse alarm?
  [ask turtles[move-alarm]]
  [ask turtles[move-normal]]
...
to move-alarm
  ...
  set next-patch one-of ((neighbors4 with [pcolor != white and any? turtles-here = false]) with-min [item ([chosen-exit] of myself) dist-exit-list])
      carefully
      [
        face next-patch
        move-to next-patch
...
to remove-evacuees
  ...
  [
    ...
    if pcolor = colour
    [
      set count-evacuees-list (replace-item i count-evacuees-list (item i count-evacuees-list + 1))
      die
    ]
    ])
  ]

  set count-allevacuees sum count-evacuees-list
...
to draw-building
  ...
  set floor-patches (patches with [pcolor = black])

end


to show-evac-routes [exit-num]

  ifelse exit-num > 0
  [

  ask floor-patches with [(item (exit-num - 1) dist-exit-list) > 0]
    [
      set pcolor scale-color yellow (item (exit-num - 1) dist-exit-list ) (max [(item (exit-num - 1) dist-exit-list)] of floor-patches) 0
    ]
  ]
  [
    ask floor-patches with [dist-allexits > 0]
    [
      set pcolor scale-color yellow dist-allexits (max [dist-allexits] of floor-patches) 0
    ]
  ]
...
to show-staffonly
  ...
  ask floor-patches with [staffonly?]
  [set pcolor yellow]
...
to nearest-exit
  ...
  ask floor-patches
  [
    let i (position (min dist-exit-list) dist-exit-list)
    set pcolor (item i [green blue red])
  ]
...
to show-chosen-exit
  ...
  [
    let i chosen-exit
    set color (item i [green blue red])
  ]

  wait 5

  ask staffs [set color cyan]
  ask publics  [set color pink]
...
"" ""
PENS
"default" 1.0 0 -13840069 true "" "plot (item 0 count-evacuees-list)"
"pen-1" 1.0 0 -13345367 true "" "plot (item 1 count-evacuees-list)"
"pen-2" 1.0 0 -2674135 true "" "plot (item 2 count-evacuees-list)"
"pen-3" 1.0 0 -16777216 true "" "plot count-allevacuees"
...
944
342
Proportion Evacuated by Type
Tick
Proportion Evacuated
0.0
100.0
0.0
1.0
...
"" ""
PENS
"default" 1.0 0 -2064490 true "" "plot (num-publics - (count publics)) / num-publics"
"pen-1" 1.0 0 -11221820 true "" "plot (num-staffs - (count staffs)) / num-staffs"
"pen-2" 1.0 0 -16777216 true "" "plot count-allevacuees / (num-publics + num-staffs)"
