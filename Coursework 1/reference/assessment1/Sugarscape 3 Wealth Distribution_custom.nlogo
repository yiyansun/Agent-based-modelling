globals [
  ...
  ;Add wealth measures (total)
  mean-sugar
  median-sugar
  sd-sugar

  ;Add wealth measures (classes)
  mean-lowviz
  mean-hiviz
  median-lowviz
  median-hiviz
...
to go
  ...
  ask turtles [
    ...
    turtle-eat
    set age (age + 1)
    if sugar <= 0 [               ;; If turtle reaches zero sugar, then...
      hatch 1 [ turtle-setup ]    ;; - hatch new turtle (to replace them)
      update-starve-count         ;; - update the starve counter to count the death
      die                         ;; - die
    ]
    if age > max-age [
      hatch 1 [ turtle-setup ]
      update-age-death-count
      die
    ]
    run visualization
  ]
...
to update-lorenz-and-gini
  ...
  set lorenz-points []
  repeat num-people [            ;; repeats the following commands once for each turtle
...
to update-starve-count    ;; custom procedure to count the number of turtles who have died by losing sugar
  set starve-count (starve-count + 1)
end

to update-age-death-count
  set age-death-count (age-death-count + 1)
end

to update-mean-sugar

  ;Totals
  set mean-sugar (mean [sugar] of turtles)
  set median-sugar (median [sugar] of turtles)
  set sd-sugar (standard-deviation [sugar] of turtles)

  ;By class
  ;Vision
  set mean-lowviz (mean [sugar] of turtles with [vision < 3])
  set mean-hiviz (mean [sugar] of turtles with [vision > 4])

  set median-lowviz (median [sugar] of turtles with [vision < 3])
  set median-hiviz (median [sugar] of turtles with [vision > 4])

  set sd-lowviz (standard-deviation [sugar] of turtles with [vision < 3])
  set sd-hiviz (standard-deviation [sugar] of turtles with [vision > 4])

  ;Metabolism
  set mean-lowmtb (mean [sugar] of turtles with [metabolism = 1])
  set mean-himtb (mean [sugar] of turtles with [metabolism = 4])

  set median-lowmtb (median [sugar] of turtles with [metabolism = 1])
  set median-himtb (median [sugar] of turtles with [metabolism = 4])

  set sd-lowmtb (standard-deviation [sugar] of turtles with [metabolism = 1])
  set sd-himtb (standard-deviation [sugar] of turtles with [metabolism = 4])
...
to-report starve-reporter
  report starve-count
end

to-report age-death-reporter
  report age-death-count
end

to-report mean-sugar-reporter
  report mean-sugar
end

to-report median-sugar-reporter
  report median-sugar
end

to-report sd-sugar-reporter
  report sd-sugar
end
...
"" ""
PENS
"Mean" 1.0 0 -13345367 true "" "plot mean-sugar"
"Median" 1.0 0 -2064490 true "" "plot median-sugar"
"Std. dev." 1.0 0 -7500403 true "" "plot sd-sugar"
...
"" ""
PENS
"Low vision" 1.0 0 -204336 true "" "plot mean-lowviz"
"High vision" 1.0 0 -10146808 true "" "plot mean-hiviz"
...
@#$#@#$#@
<experiments>
  <experiment name="test-wealth-dist-by-endowment" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="300"/>
  ...
  <experiment name="test-wealth-dist-by-vision" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    ...
    <metric>mean-sugar</metric>
    <metric>median-sugar</metric>
    <metric>mean-lowviz</metric>
    <metric>mean-hiviz</metric>
    <metric>median-lowviz</metric>
    <metric>median-hiviz</metric>
    <metric>sd-lowviz</metric>
    <metric>sd-hiviz</metric>
    <metric>mean-lowmtb</metric>
    <metric>mean-himtb</metric>
    <metric>median-lowmtb</metric>
    <metric>median-himtb</metric>
    <metric>sd-lowmtb</metric>
    <metric>sd-himtb</metric>
    <metric>starve-count</metric>
    <metric>age-death-count</metric>
    <enumeratedValueSet variable="maximum-sugar-endowment">
      <value value="25"/>
  ...
  <experiment name="mean-sugar-by-max-endowment" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>mean-sugar</metric>
    ...
    <steppedValueSet variable="maximum-sugar-endowment" first="50" step="10" last="1000"/>
    <enumeratedValueSet variable="visualization">
      <value value="&quot;color-agents-by-vision&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-population">
  ...
  <experiment name="gini-by-max-endowment-1000" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>mean-sugar</metric>
